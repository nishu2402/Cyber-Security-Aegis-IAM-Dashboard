...
| 6 | [🗺️ MITRE ATT&CK Coverage](#mitre-coverage) |
| 7 | [📊 Posture Scoring Engine](#posture-engine) |
| 8 | [🖥️ Dashboard Modules](#dashboard-modules) |
...
| 17 | [🧪 Verified Test Coverage](#test-coverage) |
| 18 | [🛣️ Future Roadmap](#future-roadmap) |
...
|---|:---:|---|
| `/` | GET | Main HUD |
| `/upload` | POST | Upload IAM JSON |
| `/init_demo` | POST | Load demo dataset |
| `/reset` | POST | Clear session + diff baseline |
| `/graph` | GET | Trust-relationship graph |
| `/playbook` | GET | Dynamic remediation playbooks |
| `/intel` | GET | Cloud export instructions |
...
| 🚫 **No code execution** | Uploaded JSON is parsed only — never `eval`d, never imported |
...
| ✅ Path-traversal `sim` ID | Rejected by allowlist |
